...
CREATE TABLE `order_detail` (
  `order_detail_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '订单明细表编号',
  `order_main_id` int(11) NOT NULL COMMENT '订单主表编号',
  `product_id` int(11) NOT NULL COMMENT '商品编号',
...
CREATE TABLE `product_info` (
  ...
  `product_type_id` int(11) NOT NULL COMMENT '商品类别编号',
  `user_id` varchar(60) NOT NULL COMMENT '用户编号',
...
CREATE TABLE `product_type` (
  `product_type_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '商品类别编号',
...
CREATE TABLE `school_info` (
  `school_info_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '校内信息编号',
...
CREATE TABLE `user_detail_info` (
  `user_detail_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '用户详细信息编号',
...
CREATE TABLE `user_info` (
  ...
  `user_head_img_url` varchar(500) NOT NULL COMMENT '用户头像',
  PRIMARY KEY (`user_id`),
  UNIQUE KEY `user_Id` (`user_id`)
...
ALTER TABLE favorite ADD CONSTRAINT `fk_favorite_userId` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`user_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
ALTER TABLE order_main ADD CONSTRAINT `fk_orderMain_userId` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`user_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
ALTER TABLE product_info ADD CONSTRAINT `fk_procuctInfo_userId` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`user_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
ALTER TABLE user_detail_info ADD CONSTRAINT `fk_userDetailInfo_userId` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`user_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
