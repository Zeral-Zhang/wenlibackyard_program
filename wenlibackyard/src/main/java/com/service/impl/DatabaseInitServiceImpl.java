...
public class DatabaseInitServiceImpl implements IDatabaseInitService {
	...
	private boolean isDataBaseInit(DatabaseInitInfo initInfo) {
		...
		BasicConfig config = basicConfigService.findByCode(initInfo.getInitCode());
		return config != null;
	}
	...
	private void updateTables(List<DatabaseUpdateInfo> updateInfoList, String initCode) {
		log.info("updateTables");
		if (null == updateInfoList || updateInfoList.isEmpty()) {
			return;
		}
		try {
			for (DatabaseUpdateInfo info : updateInfoList) {
				...
				if (version != null) {
					if (Float.parseFloat(version.getValue()) >= Float.parseFloat(info.getUpdateVersion())) {
						continue;
					}
				} else {
	...
	private void createProcedure(List<String> procedurePaths) {
		for (String procPath : procedurePaths) {
			...
			try {
				...
				throw new RuntimeException("执行sql文件：" + procPath + "时出错！", e);
			}
		}
	}
	...
	String getFileText(String classPath) {
		...
		try {
			...
			throw new RuntimeException("文件读取失败：" + classPath, e);
		}
		return str;
	}
